<template>
  <v-container class="mt-8" style="margin-left: 300px">
    ...
    <p class="text-subtitle-1 mb-0">桌号：{{ $store.state.table }}</p>
    ...
    <v-sheet width="520">
      <v-radio-group v-model="payment">
        <v-row>
          ...
          ></v-col>
          <v-col lg="9"><p class="mt-2 font-weight-bold">现金支付</p> </v-col>
          <v-col lg="1">
...
export default {
  ...
  created() {
    ...
    this.orderPrice = this.$store.state.order.foodValue;
    this.getRealOrder()
  },
  methods: {
    getRealOrder() {
